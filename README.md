...
3. **Adaptive discovery** — for each SNI the proxy resolves every
   ...
   not the address it worked on, so when a later connection's own target
   turns out to be range-blocked the proxy falls back into discovery and
   rotates onto a reachable address instead of giving up.
