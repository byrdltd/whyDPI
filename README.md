...
2. **ClientHello shaping** — the proxy reads the *complete* ClientHello,
   reassembling it into one record when the client split it across several
   TLS records (modern Chrome does this with its large post-quantum +
   Encrypted-ClientHello hello), identifies the SNI, then applies a
   fragmentation *strategy* before forwarding the bytes upstream.
3. **Adaptive discovery** — for each SNI the proxy resolves every
   ...
   that just failed.  Winners are persisted to `/run/whydpi/strategies.json`.
   A cached `passthrough` verdict records only *that* passthrough worked,
   not the address it worked on.  Before the proxy commits the client to
   that shortcut it peeks for a TLS ServerHello; a connect failure *or* a
   RST/empty after ClientHello (SNI-based DPI on a previously clean name)
   drops back into discovery on the same connection — fragmentation on
   this address, then rotation onto another if the whole range is poisoned.
4. **DoH forwarding** — the optional DNS stub forwards every query as a
   ...
   (`dns.neutralize_ech`), withholding the advertised ECH config so every
   client — Chrome included — emits a cleartext SNI rather than routing an
   Encrypted ClientHello to the CDN's public-name edge.  This keys on
   record type only, never on a hostname, so it stays site-free.
...
| `passthrough`    | Forward unchanged (probed first for new SNIs; final fallback in discovery) |

## Installation

On Linux a graphical install opens the first-run acceptable-use dialog
in the current desktop session.  Login autostart stays opt-in from the
tray's **Launch whyDPI on login** checkbox.  Headless / CLI-only:
`sudo systemctl enable --now whydpi`.

On Windows the installer can auto-launch the app; launching it starts the
engine.

### Arch Linux (AUR)

[`whydpi`](https://aur.archlinux.org/packages/whydpi), built from the latest release tag:
...
After launch the tray checks GitHub Releases (at most once per day) and,
if a newer version exists, shows a toast plus an **Install whyDPI
vX.Y.Z…** menu item.  Confirming downloads `whydpi-*-setup.exe` from
that release (SHA-256 checked when `SHA256SUMS.txt` is published) and
runs it.  Scoop installs use `scoop update whydpi` instead.  Already-
installed 1.0.2 copies do not have this checker; install a newer build
once, then later updates are offered in the tray.
...
# use the exact filename you downloaded; match the line in SHA256SUMS.txt
```
...
- A success in `whydpi probe` means the upstream produced a valid TLS
  ...
  rejected explicitly.
- IPv6 HTTPS is fully proxied.  Disable with `net.ipv6_enabled = false` if
  the upstream breaks IPv6.
