...
# Version defaults to pyproject.toml; override with -Version or WHYDPI_VERSION.
#
# Produces:
...
try {
    ...
    try {
        ...
        & $py -m PyInstaller --onedir --windowed --uac-admin `
            ...
            --hidden-import whydpi.ui.update `
            --hidden-import whydpi.ui.prefs `
            --hidden-import whydpi.i18n `
            --hidden-import whydpi.strings `
            --name whydpi-tray `
            --distpath build\dist `
            --workpath build\work `
            --specpath build `
