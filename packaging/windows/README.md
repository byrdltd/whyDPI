...
2. Calls PyInstaller twice — once for the tray entry and once for the
   CLI — in **`--onedir`** mode (not onefile: WinDivert races the
   onefile temp-dir cleanup on exit).  Flags include:
   * `--onedir --windowed --uac-admin` (tray),
   * `--onedir --console --uac-admin` (CLI),
   * `--icon=assets/favicon.ico`,
   * `--collect-all pydivert` so the bundled WinDivert driver (`.sys`)
     ships inside the folder,
...
Version defaults to `pyproject.toml`; override with `-Version x.y.z` or
`$env:WHYDPI_VERSION = "x.y.z"`.  The script mirrors the CI job: onedir
PyInstaller builds plus Inno Setup, output under `packaging\windows\dist\`.
...
whyDPI does not run `netsh` or install any NRPT rule — all DNS work is
transient and lives at the packet layer.  That means a crash, a kill -9
or a `systemctl stop` leaves the OS's persistent DNS configuration
...
## Upgrades and `WinDivert64.sys` (DeleteFile code 5)

WinDivert loads `WinDivert64.sys` as a kernel driver.  Windows keeps
that file mapped after the tray process exits, so a naive overwrite
during upgrade/reinstall fails with **DeleteFile failed; code 5 /
Access denied**.  The Inno script therefore:

1. Force-stops `whydpi-tray.exe` / `whydpi.exe` and `sc stop WinDivert`
   in `PrepareToInstall` / `InitializeUninstall`.
2. Marks the onedir trees `restartreplace` / `uninsrestartdelete` so a
   still-locked `.sys` is swapped on the next reboot instead of aborting
   Setup.

If an older installer is already stuck on that dialog: **Cancel**,
reboot (this unloads the driver), then run the new setup.
