...
- Author/committer: `byrdltd <byrdltd@users.noreply.github.com>`.
- Agents: prefer `git commit-tree` over `git commit` (see
  [`.cursor/rules/git-authorship.mdc`](.cursor/rules/git-authorship.mdc)).
- After committing, verify: `git log -1 --format=%B | grep -i co-authored`
  must print nothing.
...
Or with pre-commit: `pre-commit install --hook-type commit-msg`.
