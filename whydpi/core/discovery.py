...
_PEER_GONE = (errno.ECONNRESET, errno.EPIPE, errno.ECONNABORTED)


def _peek(sock: socket.socket, min_bytes: int, timeout_s: float) -> bytes:
    """Read up to *min_bytes* or until the peer goes silent.

    A TCP RST/abort after we sent ClientHello is the textbook DPI
    signature — the TLS endpoint never spoke.  Treat it as an empty
    preview so callers classify ``DPI_BLOCK`` rather than a transient
    ``recv-failed`` (which would skip address rotation).
    """
...
def probe_strategy(
    ...
    *,
    ...
    """Connect, apply *strategy* to ClientHello, peek for a TLS reply.

    On success the socket is left open: *preview* has already been
    consumed and must be spliced to the client before relaying.  On
    failure the socket is closed and the caller may try another
    strategy against the same ClientHello — the client is untouched.
    """
    return _probe_one(
        strategy,
        dest_ip=dest_ip,
...
def _discover_at_target(
    ...
    *,
    ...
    # A caller that already burned passthrough on this address (the
    # proxy's cached-passthrough shortcut just RST'd) must not spend
    # another RTT on the same unshaped send.  Fragmentation and a
    # different cached layer still run.
    if cached is not None and cached.layer == "passthrough" and not include_passthrough:
        cached = None
    ...
    if include_passthrough and probe_passthrough_first and cached is None:
        strategy, sock, preview, reason = _probe_one(
            _PASSTHROUGH,
            dest_ip=target.ip,
            dest_port=target.port,
    ...
    if include_passthrough and (not probe_passthrough_first or cached is not None):
        strategy, sock, preview, reason = _probe_one(
            _PASSTHROUGH,
            dest_ip=target.ip,
            dest_port=target.port,
...
def discover_upstream(
    *,
    ...
    passthrough_on_primary: bool = True,
) -> DiscoveryResult:
    """Pick a working strategy for one client connection.

    *passthrough_on_primary* is False when the caller already probed
    unshaped passthrough on the client-chosen address (and it RST or
    went silent).  Fragments still run there; rotated addresses get a
    fresh passthrough probe — a different anycast range may not need
    shaping even when this one did.
    """
    ...
    result = _discover_at_target(
        ...
        accept_alert=accept_alert,
        include_passthrough=passthrough_on_primary,
    )
