...
def _add_common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("-v", "--verbose", action="store_true", help=t("cli.help.verbose"))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("start", help=t("cli.help.start"))
    _add_common(p)
    p.add_argument("--configure-dns", action="store_true",
                   help=t("cli.help.configure_dns"))
    p.add_argument("--probe-targets", nargs="*", default=[],
                   help=t("cli.help.probe_targets"))
    p.add_argument("--dns-mode", choices=["doh", "altport", "off"],
                   help=t("cli.help.dns_mode"))
    p.set_defaults(func=cmd_start)

    p = sub.add_parser("stop", help=t("cli.help.stop"))
    _add_common(p)
    p.set_defaults(func=cmd_stop)

    p = sub.add_parser("dns-configure", help=t("cli.help.dns_configure"))
    _add_common(p)
    p.set_defaults(func=cmd_dns_configure)

    p = sub.add_parser("dns-restore", help=t("cli.help.dns_restore"))
    _add_common(p)
    p.set_defaults(func=cmd_dns_restore)

    p = sub.add_parser("probe", help=t("cli.help.probe"))
    _add_common(p)
    p.add_argument("targets", nargs="+", help=t("cli.help.targets"))
    p.set_defaults(func=cmd_probe)

    p = sub.add_parser("cache", help=t("cli.help.cache"))
    _add_common(p)
    cache_sub = p.add_subparsers(dest="subcmd", required=True)
    cache_sub.add_parser("list", help=t("cli.help.cache_list"))
    cache_sub.add_parser("clear", help=t("cli.help.cache_clear"))
    forget = cache_sub.add_parser("forget", help=t("cli.help.cache_forget"))
    forget.add_argument("hosts", nargs="+", help=t("cli.help.cache_forget_hosts"))
