...
def show_status_window(cache_file: Path) -> None:
    ...
    for host, ent in cache.entries_snapshot():
        rows.append(
            (
                ...
                ent.last_failure_kind or "—",
            ),
        )

    root = tk.Tk()
    root.title(t("status.title"))
    ...
    tk.Label(root, text=t("status.cache_file", path=path), font=("sans-serif", 9)).pack(
    ...
    cols = ("host", "strategy", "ok", "fail", "last_ok", "last_fail")
    tree = ttk.Treeview(frame, columns=cols, show="headings", height=14)
    tree.heading("host", text=t("status.col.host"))
    tree.heading("strategy", text=t("status.col.strategy"))
    tree.heading("ok", text=t("status.col.ok"))
    tree.heading("fail", text=t("status.col.fail"))
    tree.heading("last_ok", text=t("status.col.last_ok"))
    tree.heading("last_fail", text=t("status.col.last_fail"))

    tree.column("host", width=200)
    tree.column("strategy", width=140)
    tree.column("ok", width=60, anchor="e")
    tree.column("fail", width=60, anchor="e")
    tree.column("last_ok", width=140)
    tree.column("last_fail", width=120)
    ...
    if not rows:
        tree.insert("", "end", values=("—", t("status.empty"), "—", "—", "—", "—"))

    tk.Button(root, text=t("status.close"), command=root.destroy).pack(pady=(8, 2))
    ...
    tk.Label(
        root,
        text=t("status.footer", version=_version),
        font=("sans-serif", 8),
        fg="#666666",
    ).pack(pady=(0, 6))
