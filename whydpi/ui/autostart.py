...
* **Linux** — autostart is off by default.  Packaging (AUR / .deb /
  .rpm) and ``install.sh`` ship a launcher under
  ``/usr/share/applications`` so the user can click the app after
  install; they do **not** drop a system-wide XDG autostart file.
  Opting in writes ``whydpi-tray.desktop`` under
  ``$XDG_CONFIG_HOME/autostart/``.  Opting out deletes it.  A leftover
  system-wide ``/etc/xdg/autostart`` entry from older packages is still
  honoured: opting out then writes a ``Hidden=true`` override so the
  session manager ignores it without needing root.
...
def _localized_desktop_identity(*, comment_key: str) -> dict[str, str]:
    """English ``Name``/``Comment`` plus ``Name[lang]`` for every catalog."""
    en = CATALOGS["en"]
    out = {
        "Name": en["desktop.name"],
        "GenericName": en["desktop.generic"],
        "Comment": en[comment_key],
    }
    for code, table in CATALOGS.items():
        if code == "en":
            continue
        out[f"Name[{code}]"] = table["desktop.name"]
        out[f"GenericName[{code}]"] = table["desktop.generic"]
        out[f"Comment[{code}]"] = table[comment_key]
    return out
...
def _find_tray_exec() -> str | None:
    ...
    back to ``python -m whydpi.ui.tray`` keeps us working in a dev
    checkout where the console script isn't registered yet.
    """
    for cand in ("/usr/bin/whydpi-tray", "/usr/local/bin/whydpi-tray"):
...
def _linux_set(enabled: bool) -> bool:
        ...
        if system is not None:
            ...
            if user.exists():
                try:
                    ...
                    return False
            return True
        # Write a user-level XDG autostart entry (package and pip alike).
        exec_line = _find_tray_exec() or "whydpi-tray"
        kv = {
            "Type": "Application",
            **_localized_desktop_identity(comment_key="desktop.autostart_comment"),
            "Exec": exec_line,
            "Icon": "whydpi",
            "Terminal": "false",
            "Categories": "Network;",
    ...
    if system is not None:
        ...
        try:
            ...
            return False
        return True
    # No system-wide entry: just remove our own user file.
    if user.exists():
        try:
            user.unlink()
        except OSError as exc:
...
def ensure_menu_entry() -> bool:
    ...
    kv = {
        "Type": "Application",
        **_localized_desktop_identity(comment_key="desktop.comment"),
        "Exec": exec_line,
        "Icon": "whydpi",
        "Terminal": "false",
        "Categories": "Network;",
