...
class DNSSettings:
    ...
    altport_server: str = ""
    altport_port: int = 0
    # Encrypted ClientHello (ECH) encrypts the real SNI inside the TLS
    # handshake.  whyDPI's entire mechanism depends on *observing* the SNI
    # so it can fragment and IP-rotate on it; an ECH hello also routes to
    # the ECH public-name edge (e.g. ``cloudflare-ech.com``) rather than
    # the target zone, so an ECH browser (Chrome) fails on a range-blocked
    # CDN where a cleartext-SNI browser (Firefox) succeeds.  When enabled
    # (default) the stub answers HTTPS/SVCB (type 65 / 64) queries with
    # NODATA, withholding the advertised ``ech=`` config so every client
    # falls back to a cleartext SNI and the existing rotation handles the
    # block uniformly.  This is fully site-free — it keys on record type,
    # never on a hostname or list.
    neutralize_ech: bool = True
...
class TLSSettings:
    ...
    # trick.  Consumer-edge DPI boxes often sit 3-5 hops from a
    ...
    # When an SNI is unseen, probe passthrough before fragmentation so
    # DPI-clean sites (payment gateways, CDNs) are not reshaped needlessly.
    ...
    # Hosts always forwarded unchanged (user pre-seed).  Successful
    # passthrough probes are cached per-SNI automatically.
...
def _merge_dns(base: DNSSettings, data: dict) -> DNSSettings:
    ...
    if "neutralize_ech" in data:
        changes["neutralize_ech"] = bool(data["neutralize_ech"])
...
def _merge_tls(base: TLSSettings, data: dict) -> TLSSettings:
    ...
    if "probe_passthrough_first" in data:
        changes["probe_passthrough_first"] = bool(data["probe_passthrough_first"])
...
def _apply_env(s: Settings) -> Settings:
    dns = replace(
        ...
        neutralize_ech=_env_bool("NEUTRALIZE_ECH", s.dns.neutralize_ech),
    )
    ...
    tls = replace(
        ...
        probe_passthrough_first=_env_bool(
            "PROBE_PASSTHROUGH_FIRST", s.tls.probe_passthrough_first,
        ),
