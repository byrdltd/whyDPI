...
# Tray desktop integration (application menu + hicolor icons).
# Login autostart is a tray opt-in, not a packaged default.
...
install -Dm755 packaging/linux/first-run-launch.sh \
    /usr/lib/whydpi/first-run-launch.sh
rm -f /etc/xdg/autostart/whydpi-tray.desktop
...
echo "✅ Tray desktop integration installed (launch from the application menu)"
echo ""
...
# Open the first-run consent dialog in the installer's desktop session.
...
echo "  whydpi-tray                    # system tray icon (also in the application menu)"
